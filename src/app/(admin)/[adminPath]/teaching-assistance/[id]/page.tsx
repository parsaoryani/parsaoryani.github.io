...
const breadcrumbs: Breadcrumb[] = [
  ...
  { label: "Teaching Experience", href: "/x7k2-console/teaching-assistance" },
  { label: "Edit" },
]
...
export default function EditTeachingPage() {
  ...
  useEffect(() => {
    fetch(`/api/teaching-assistance/${params.id}`)
      .then(r => r.json())
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(`/api/teaching-assistance/${params.id}`, {
        method: "PUT",
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/teaching-assistance/${params.id}`, { method: "DELETE" })
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <FieldGroup label="Professor" required>
          <Input name="professor" required defaultValue={item.professor} />
        </FieldGroup>
      </div>
      <div className="grid grid-cols-2 gap-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <FieldGroup label="Technologies / Topics">
          <Input name="technologies" defaultValue={item.technologies || ""} placeholder="e.g. Machine Learning, Python" />
        </FieldGroup>
      ...
      <SectionHeader title="Teaching Highlights" description="One highlight per line" />
      <FieldGroup label="Highlights" hint="Each line becomes a separate bullet point">
        <Textarea name="highlights" rows={4} defaultValue={item.highlights?.join("\n") || ""} placeholder="Led weekly lab sessions&#10;Graded 50+ assignments&#10;Developed course materials" />
      </FieldGroup>
    </EditorLayout>
  )
}